...
def printErrorMessages(errorList):
    if errorList == []: #End of validation, print results to terminal
            print('Validation is complete. No errors were found. \n')
    else:
        print('Validation is complete. The following errors were found:')
        print('\n'.join(errorList) + '\n')
...
def validateSpreadsheet(inputXLSX):
    ...
    if validateTable(sheet) == None:
        print('An incomplete control record is present or the spreadsheet is blank.')
    elif validateTable(sheet) == False:
        print('The worksheet name must be T025X, T026X, or TU130. \n')
    else:
        ...
        printErrorMessages(errorMessages) #Print error messages
